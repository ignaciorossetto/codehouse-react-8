...
function CartModal() {
  ...
  return (
    <>
      ...
      <Modal size='md' show={show} onHide={handleClose} style={{ width: "100%" }}>
        <Modal.Header closeButton>
          <Modal.Title>{cart.length>0 ? `Carrito!` : `Carrito Vacio!`}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {cart.length === 0 ? "": cart.sort((a,b)=> a.product_id - b.product_id).map((product) => {
            return (
              <Card
